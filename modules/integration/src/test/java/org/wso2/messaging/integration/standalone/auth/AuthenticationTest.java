...
public class AuthenticationTest {
    ...
    @Test(description = "Test valid user")
    ...
    @Test(description = "Test user password with special charcters")
    ...
    @Parameters({ "broker-port", "admin-password" })
    @Test(description = "Test invalid username",
          expectedExceptions = JMSException.class)
    public void testInvalidClientUername(String port, String adminPassword) throws Exception {
        String topicName = "MyTopic1";
        InitialContext initialContext = ClientHelper
                .getInitialContextBuilder("userNotExist", adminPassword, "localhost", port).withTopic(topicName)
                .build();
        TopicConnectionFactory connectionFactory = (TopicConnectionFactory) initialContext
                .lookup(ClientHelper.CONNECTION_FACTORY);
    ...
    @Test(description = "Test invalid user password",
          expectedExceptions = JMSException.class)
    public void testInvalidClientPassword(String port, String adminUsername) throws Exception {
        String topicName = "MyTopic1";
        InitialContext initialContext = ClientHelper
