...
public class PermissionStore {
    ...
    private boolean authorizeResourceGroup(String resourceGroup, Set<String> userGroups,
        ...
        return userGroupPermissions.entrySet()
                                   .stream()
                                   .filter((entry) -> (entry.getValue() & permission) == permission &&
                                           userGroups.contains(entry.getKey()))
                                   .findFirst()
                                   .isPresent();
    }

    /**
     * Clean the inmemory permissions.
     * This will be used by passive node when activation inorder to make sure consistent with current state of
     * database.
     */
    public void clearAllPermissions() {
        resourcePermission.getFastTopicMatcherMap().clear();
        resourcePermission.getResourcePermissions().clear();
        resourceGroupPermission.getResourceGroupPermissions().clear();
